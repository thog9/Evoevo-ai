...
░░░╚═╝░░░╚═╝░░╚═╝░╚════╝░░╚═════╝░░░░╚═╝░░░░╚════╝░░╚════╝░╚══════╝╚═╝░░╚═╝░╚═════╝░╚═════╝░
    ...
    print_border("EVOEVO AI", Fore.GREEN)
...
async def run_account(language: str):
    from scripts.account import run_account
    await run_account(language)

async def run_agent(language: str):
    from scripts.agent import run_agent
    await run_agent(language)

async def run_autoreff(language: str):
    from scripts.autoreff import run_autoreff
    await run_autoreff(language)

async def run_bind(language: str):
    from scripts.bind import run_bind
    await run_bind(language)

async def run_blindboxes(language: str):
    from scripts.blindboxes import run_blindboxes
    await run_blindboxes(language)

async def run_topics(language: str):
    from scripts.topics import run_topics
    await run_topics(language)

async def run_withdraw(language: str):
    from scripts.withdraw import run_withdraw
    await run_withdraw(language)
...
def get_available_scripts(language):
    scripts = {
        'vi': [
            {"name": "1. Quản lý Account (Profile & Favorites)", "value": "account"},
            {"name": "2. Quản lý Agent + Tạo Agents", "value": "agent"},
            {"name": "3. Auto Referral (Tạo ví mới)", "value": "autoreff"},
            {"name": "4. Register & Bind", "value": "bind"},
            {"name": "5. Blindbox Management", "value": "blindboxes"},
            {"name": "6. Topics & Oracle", "value": "topics"},
            {"name": "7. Withdraw Funds", "value": "withdraw"},
          
            {"name": "X. Thoát", "value": "exit"},
        ],
        'en': [
            {"name": "1. Account Management (Profile & Favorites)", "value": "account"},
            {"name": "2. Agent Management + Create Agents", "value": "agent"},
            {"name": "3. Auto Referral (Create new wallets)", "value": "autoreff"},
            {"name": "4. Register & Bind", "value": "bind"},
            {"name": "5. Blindbox Management", "value": "blindboxes"},
            {"name": "6. Topics & Oracle", "value": "topics"},
            {"name": "7. Withdraw Funds", "value": "withdraw"},
